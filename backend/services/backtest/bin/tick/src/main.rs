...
pub fn stream_binary_to_redis(bin_path: &str, stream_key: &str, redis_url: &str) {
    ...
    let mut con = client.get_connection().expect("Error al conectar con Redis");
    ...
    println!("Reseteado: Stream '{}' limpio para nueva ejecución.", stream_key);
    ...
    while reader.read_exact(&mut buffer).is_ok() {
        ...
        pipe.cmd("XADD")
            .arg(stream_key)
            .arg("*")
            .arg("trade_id").arg(trade.trade_id)
            .arg("price").arg(trade.price)
            .arg("qty").arg(trade.qty)
            .arg("timestamp").arg(trade.timestamp_ms)
            .arg("is_buyer_maker").arg(trade.is_buyer_maker);

        count += 1;

        // Si alcanzamos el tamaño del lote, vaciamos el pipeline hacia la red
        if count % batch_size == 0 {
            let _: () = pipe.query(&mut con).expect("Error al ejecutar pipeline en Redis");
    ...
    if count % batch_size != 0 {
        let _: () = pipe.query(&mut con).expect("Error al vaciar el último lote en Redis");
    }

    println!("Streaming finalizado. Se han enviado {} ticks al stream '{}'.", count, stream_key);
}
